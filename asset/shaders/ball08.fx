...
<
   ...
   string UIWidget = "Color";
   bool UIVisible =  true;
> = float4( 0.99, 0.99, 0.99, 1.00 );
float4 fvDiffuse
<
   string UIName = "fvDiffuse";
   string UIWidget = "Color";
   bool UIVisible =  true;
> = float4( 0.19, 0.19, 0.18, 1.00 );
...
texture base_Tex
<
   string ResourceName = "..\\textures\\poolb8.jpg";
>;
...
float4 Textured_Phong_Pass_0_Pixel_Shader_ps_main( PS_INPUT Input ) : COLOR0
{
   ...
   return ( fvAmbient ) * fvBaseColor + diffuseAttn * fvDiffuse + specularAttn * fvSpecular;
}
