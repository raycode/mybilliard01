...
//	float sx2= 0.38f;
	...
	{
		// display title
		sprintf_s(buf, 256, "Profiler (FPS: %g)",computeFPS());
...
#ifdef NX_ENABLE_PROFILER_COUNTER
				sprintf_s(buf, 256, "%4d    %4.0f  %4.0f %4.0f %6d %6d %7d", z.callCount, z.percent, precTotal, percSelf, z.hierTime, z.selfTime,z.counter);
#else
				sprintf_s(buf, 256, "%4d    %4.0f  %4.0f %4.0f %10d %10d", z.callCount, z.percent, precTotal, percSelf, z.hierTime, z.selfTime);
#endif
...
//				GLFontRenderer::print(sx2, sy, fontHeight, buf, true, 11, true);
		...
		// display title
		sprintf_s(buf, 256, "FPS: %g",computeFPS());
//		GLFontRenderer::print(sx, sy, fontHeight, buf, false, 11, true);
		}
